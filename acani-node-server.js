...
server.addListener("connection", function (conn) {
  ...
  conn.addListener("message", function (message) {
    ...
    } else if (uid && message.content && message.to_uid_public) {
      console.log("SENDING FROM: " + uid + " TO PUBLIC: " + message.to_uid_public + ": " + message.content);
      // SEND TO USER
      client_pub.publish([message.to_uid_public, uid_public].sort().join("_"), message.content);
    } else if (uid && message.content && message.to_room) {
      console.log("SENDING FROM: " + uid + " TO ROOM: " + message.to_room + ": " + message.content);
      // SEND TO ROOM
      client_pub.publish(message.to_room, message.content);
